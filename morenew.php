<?php
	...
	$page2=$page*10;
		...
		$qr2="SELECT * FROM tintuyendung_vieclam ORDER BY ngaydang DESC LIMIT $page2,10";
		...
		if(mysqli_num_rows($r2)>0)
	   {
		...
		while($row = mysqli_fetch_array($r2, MYSQL_ASSOC))
		{
				...
				array_push($mang, new congviec($row["macv"],$matd,$views,$row["tencv"],$tenct,$rowss['diachi'],$rowss['quymo'],$rowss['nganhnghe'],$rowss['mota'],$row["mucluong"],$row["diadiem"],$row["hannophoso"],$motacv,$row["bangcap"],$row["dotuoi"],$row["ngoaingu"],$row["gioitinh"],$row["yeucaukhac"],$row['tenkynang'],$rss["photo"],$rws["phone"],"","",$row['ngaydang']));
			}	
		}	
			 mysqli_close($con);
