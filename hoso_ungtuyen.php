<?php
    ...
    if(isset($_POST['macv']) || isset($_POST['status'])) {
	  $macv=$_POST['macv'];
		$i=$_POST['status'];
	  
       $qr="SELECT * FROM hoso_ungtuyen_nhatuyendung WHERE macv = '$macv' AND status = '$i'";
		...
		while($row = mysqli_fetch_array($r, MYSQL_ASSOC))
		{
			...
			$ss = mysqli_query($con,"SELECT * FROM hoso_nguoitimviec WHERE mahs = '$mahs'");
			$raw = mysqli_fetch_array($ss, MYSQL_ASSOC);
			$id=$raw['unique_id'];
